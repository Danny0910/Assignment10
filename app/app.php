...
// NOTE: these are loaded using the autoloader provided by composer
...
$app->register(new Silex\Provider\SecurityServiceProvider(), array(
    'security.firewalls' => array(
        ...
        'user' => array(
            ...
            // YOUR CODE HERE: You need to add a new route pattern for the edit blog post route.
            // YOUR CODE HERE: You need to add a new route pattern for the delete blog post route.
            'pattern' => '(^/user)|(^/blog/new-post)|(^/blog/id/.*/edit)|(^/blog/id/.*/delete)',
    ...
    'security.access_rules' => array(
        ...
        // YOUR CODE HERE: You need to add a new route pattern to ROLE mapping for the edit blog post route.
        // YOUR CODE HERE: You need to add a new route pattern to ROLE mapping for the delete blog post route.
        array('^/blog/new-post', 'ROLE_USER'),
        array('^/blog/id/.*/edit', 'ROLE_USER'),
        array('^/blog/id/.*/delete', 'ROLE_ADMIN'),
    ),
));
...
$app->get('/blog/author/{author_name}', function ($author_name) use ($app) {
    // Use the function SilexBlog\PostRepository->findByAuthor($author) to get the specified post by its id.
    $author = $app->escape($author_name);
    $posts = $app['repository.post']->findByAuthor($author);

    // Set the page title to the blog post title and author
    $page_title = 'All posts by ' . $author;

    // Pass the page title and post to twig to be rendered using the list_posts.twig template.
    return $app->render('list_posts.twig', array('page_title' => $page_title, 'posts' => $posts));
})
    ->bind('findPostsByAuthor')
    ->assert('author_name', '[\w\s]+');

// A controller to create a new blog post.
$app->match('/blog/new-post', function (Request $request) use ($app) {
    ...
    return $app->render('new-or-edit-post.twig', array('page_title' => 'New Post', 'form' => $form->createView()));
})
->bind('newPost');

// A controller to edit a blog post.
$app->match('/blog/id/{id}/edit', function (Request $request, $id) use ($app) {
    // Default data for the form.
    // YOUR CODE HERE
    // You need to look up the data for the existing Post instance that has id $id
    // and use it here for the default values.
    // Look at the code for the findPost route to see how to look up a Post instance with a given id.
    $post = $app['repository.post']->find($id);
    $data = array(
    ...
    // Only save the modified post if the form input passes all the given validation rules.
    if ($form->isValid()) {
        // Get the form input.
        $data = $form->getData();

        // Use the PostFactory to create a new Post instance.
        // You need to modify the Post instance you looked up earlier.
        // YOUR CODE HERE
        $post->setAuthor($app->escape($data['author']));
        $post->setTitle($app->escape($data['title']));
        $post->setBody($app->escape($data['body']));
        if(!$post->getPersisted()) {
    ...
    // The form data is either invalid or the form is being display for the first time.
    // So, render the form template.
    return $app->render('new-or-edit-post.twig', array('page_title' => 'Edit Post', 'form' => $form->createView()));
})
    ->bind('editPost')
    ->assert('id', '\d+');

// A controller to delete a blog post.
$app->match('/blog/id/{id}/delete', function ($id) use ($app) {
    // Look at the code for the findPost route to see how to look up a Post instance with a given id.
    $post = $app['repository.post']->find($id);

    // Use the PostRepository to delete the Post from the database.
