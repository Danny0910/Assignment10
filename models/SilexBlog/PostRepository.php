...
class PostRepository
{
    ...
    // Grab all posts by author from the database and return an array of Post objects.
    public function findByAuthor($author) {
        $db_results = $this->app['db']->fetchAll('SELECT * FROM `posts` where author = ?', array($author));
        if (empty($db_results)) {
            throw new PostNotFoundException('No posts were found in the database for author '. $author);
        }
    ...
    public function save(Post $post_object) {
        ...
        if(is_null($post_object->getId())) {
            ...
            // YOUR CODE HERE: update post (part of the assignment requirements)
            if(!$post_object->getPersisted()) {
                $this->app['db']->update('posts', array(
                    'author' => $post_object->getAuthor(),
                    'title' => $post_object->getTitle(),
                    'body' => $post_object->getBody(),
                    'modified_date' => $post_object->getModifiedDate(),
                ), array('id' => $post_object->getId()));
                $post_object->setPersisted(true);
            }
        }
    }
